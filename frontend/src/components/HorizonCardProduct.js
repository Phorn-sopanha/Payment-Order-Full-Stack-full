...
const HorizonCardProduct = ({category , heading}) => {
                    ...
                    return(
                        <Link to={"product/" + product?._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                            <div className='bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[145px]'>
                                <img src={product?.productImage[0]} className='object-scale-down h-full hover:scale-110 transition-all cursor-pointer' alt='dd'/>
                            </div>
                            <div className='p-4 grid'>
                                ...
                                <button className='text-sm bg-slate-600 hover:bg-red-600 text-white px-3 rounded-full py-0.5' onClick={(e)=>handleAddToCart(e,product?._id)}>Add to Cart</button>
                            </div>
                        </Link>
                    )
                })
